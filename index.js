...
//Link Blacklist
const linkBlacklist = ['http', 'www', '.com', '.net', '.org', '.gg']
  ...
  if (message.guild.id === server_id) {
...

    {
      ...
      if (
        ...
        cmd === `${prefix}advert` ||
        cmd === `${prefix}adverts` ||
        cmd === `${prefix}advertisement` ||
        cmd === `${prefix}advertisements`
      ) {
        var embed = new Discord.MessageEmbed()
          .setColor(color['blue'])
...

    {
        ...
        if (message.member.roles.cache.get(roleID['staff']) != undefined) {
          ...
          if (args[1] === undefined || args[1] === `1`) {
            ...
            embed
              ...
              .addField(
                `.latereg`,
                'Ping a staff member in <#709320648215887893>, and we will help you as much as we can.',
              )
            ...
            var embed2 = new Discord.MessageEmbed().setColor(color['blue']).setDescription('Invalid page number.')
      ...
      if (cmd === `${prefix}mute`) {
        ...
        if (
          ...
          person.roles.highest.position < message.member.roles.highest.position
        ) {
          let time = args[2]
      ...
      if (cmd === `${prefix}kick`) {
        ...
        if (
          ...
          message.member.roles.highest.position > person.roles.highest.position
        ) {
          reason = ''
      ...
      if (cmd === `${prefix}ban`) {
        ...
        if (
          ...
          person.roles.highest.position < message.guild.roles.cache.get(roleID['staff'])
        ) {
          var person = message.guild.member(message.mentions.users.first())
          for (i = 2; i < args.length; i++) {
            reason += args[i] + ' '
          ...
          if (reason === '') {
            var embed = new Discord.MessageEmbed().setColor(color['green']).setDescription('MUST give reason for ban.')
            message.channel.send(embed)
            return
          }
      ...
      if (cmd === `${prefix}role`) {
        if (
          ...
          message.mentions.roles.first().position < message.member.roles.highest.position
        ) {
          var person = message.guild.member(message.mentions.users.first())
          ...
          var embed = new Discord.MessageEmbed()
            ...
            .setDescription(
              message.member.toString() + ' tried assigning/removing either their highest role or any above.',
            )
...
bot.on('messageDelete', async (message) => {
  ...
  if (message.guild.id === server_id) {
    ...
    if (
      message.channel.parent === category['admin'] ||
...
bot.on('messageUpdate', async (oldMessage, newMessage) => {
  ...
  if (newMessage.guild.id === server_id) {
    if (
      oldMessage.channel.parent === category['admin'] ||
